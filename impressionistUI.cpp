...
//-------------------------------------------------
  ...
  {
    m_nSize = size;
    if (1 <= size && size <= 40) m_BrushSizeSlider->value(m_nSize);
  }

/* Get the brush line width. */
...
/* Set the brush line width. */
  void ImpressionistUI::setLineWidth(int lineWidth)
  {
    ...
    if (1 <= lineWidth && lineWidth <= 40) m_LineWidthSlider->value(m_nLineWidth);
  }

/* Get the brush line angle. */
...
/* Set the brush line angle. */
  void ImpressionistUI::setLineAngle(int lineAngle)
  {
    ...
    if (0 <= lineAngle && lineAngle <= 359) m_LineAngleSlider->value(m_nLineAngle);
  }
...
/* Set the brush alpha. */
  void ImpressionistUI::setAlpha(double alpha)
  {
    ...
    {
      printf("WARNING: Trying to set alpha to a value outside the interval [0.0, 1.0].\n");
    }
  }
